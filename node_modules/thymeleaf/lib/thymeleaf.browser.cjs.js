...
var objectUtils = require('@ultraq/object-utils');
...
new Rule('VariableExpression', Sequence(/\${/, 'Identifier', /\}/), (_ref) => {
	...
	return context => {
		let result = objectUtils.navigate(context, identifier);
	...
	process(element, attribute, attributeValue, context) {
		...
		if (/(.+=.+,)*.+=.+/.test(attributeValue)) {
			attributeValue.split(',').forEach(attribute => {
				...
				element.setAttribute(attributeParts[0], stringUtils.escapeHtml(new ExpressionProcessor(context).process(attributeParts[1])));
			});
		}
			...
			return regenerator.wrap(function _callee$(_context) {
				while (1) switch (_context.prev = _context.next) {
					...
					case 10:
						...
						fragment = dumbQuerySelector.$(`[${dialectPrefix}\\:${fragmentProcessorName}^="${fragmentName}"]`, template) || dumbQuerySelector.$(`[data-${dialectPrefix}-${fragmentProcessorName}^="${fragmentName}"`, template);
...
class TemplateEngine {
  ...
  */
	constructor() {
		...
		this.isomorphic = isomorphic;
		this.processors = arrayUtils.flatten(dialects.map(dialect => dialect.processors));
			...
			return regenerator.wrap(function _callee$(_context) {
				while (1) switch (_context.prev = _context.next) {
					case 0:
						...
						localContext = objectUtils.merge({}, context, localVariables);
